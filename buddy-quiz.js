const API_URL = 'https://valorant-api.com/v1/buddies?language=ja-JP';
...
function createQuestion(buddy) {
  const others = allBuddies.filter(b => b.name !== buddy.name);
  const wrongs = pickRandom(others, 3).map(b => b.name);
  return {
    correct: buddy,
    choices: shuffle([buddy.name, ...wrongs]),
  };
}

function buildQuestions() {
  const picked = pickRandom(allBuddies, TOTAL_QUESTIONS);
  return picked.map(buddy => createQuestion(buddy));
}
...
function showResult() {
  ...
  const messages = [
    [1.0, 'パーフェクト！ガンバディ博士👑'],
    [0.8, 'すごい！かなりのガンバディ知識だ👏'],
    [0.6, 'なかなかやるね！もう一回挑戦してみよう'],
    [0.4, 'まだまだこれから。ガンバディを集めてみよう'],
    [0,   'ガンバディ、つけてる…？'],
  ];
